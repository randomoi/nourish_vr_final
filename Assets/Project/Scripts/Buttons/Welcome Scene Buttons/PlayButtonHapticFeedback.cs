...
public class PlayButtonHapticFeedback : MonoBehaviour
{
    public Button playButton; // to be assigned in Unity inspector

    void Start()
    {
        if (playButton != null)
        {
            playButton.onClick.AddListener(TriggerHapticFeedback);
        }
    }
    ...
    void OnDestroy()
    {
        if (playButton != null)
        {
            playButton.onClick.RemoveListener(TriggerHapticFeedback);
        }
    }
}
